...
function index()
	...
	entry({"admin", "system", "fros", "check_old_version"}, call("check_old_version")).leaf = true
    entry({"admin", "system", "fros", "upgrade_fros_ipk"}, call("upgrade_fros_ipk")).leaf = true
    entry({"admin", "system", "fros", "upgrade_fros_ipk_free"}, call("upgrade_fros_ipk_free")).leaf = true
...
function check_old_version()
    ...
    local oaf_status = sys.exec("opkg list |grep oaf")
    if oaf_status == nil or oaf_status == ""  then
    	resp_json.code = 40000
    else
...
function upgrade_fros_ipk_with_type(f_type)
    local resp_json= {
        code=40005,
        data={}
    }
    ...
    luci.sys.exec("echo 'ftype = "..f_type.."' >>/tmp/lua.log");

    local url = fros_api.."?kernel="..kernel.."&arch="..board.."&type="..f_type
    luci.sys.exec("echo 'url2 = "..url.."' >>/tmp/lua.log");
...
function upgrade_fros_ipk_free()
    upgrade_fros_ipk_with_type(0)
end

function upgrade_fros_ipk()
    upgrade_fros_ipk_with_type(1)
end
